...
/*
 * fips_module private copy of lib/crypto/sha512.c
 *
 * Provides fips_lib_sha384_*() / fips_lib_sha512_*() and their HMAC
 * counterparts — module-private implementations of SHA-384/512 and
 * HMAC-SHA-384/512 library functions for use within the FIPS cryptographic
 * boundary.  These functions are not exported to vmlinux; all callers within
 * fips_module must include "lib/crypto/sha512_lib.h" and call the fips_lib_*
 * variants directly.
 *
...
static void sha512_blocks_generic(struct sha512_block_state *state,
...
void fips_lib_sha384(const u8 *data, size_t len, u8 out[SHA384_DIGEST_SIZE])
{
	...
	fips_lib_sha384_init(&ctx);
	fips_lib_sha384_update(&ctx, data, len);
	fips_lib_sha384_final(&ctx, out);
}
...
void fips_lib_sha512(const u8 *data, size_t len, u8 out[SHA512_DIGEST_SIZE])
{
	...
	fips_lib_sha512_init(&ctx);
	fips_lib_sha512_update(&ctx, data, len);
	fips_lib_sha512_final(&ctx, out);
}
				     ...
				     const struct sha512_block_state *iv)
{
	...
	if (unlikely(raw_key_len > SHA512_BLOCK_SIZE)) {
		if (iv == &sha384_iv)
			fips_lib_sha384(raw_key, raw_key_len, derived_key.b);
		else
			fips_lib_sha512(raw_key, raw_key_len, derived_key.b);
	} else {
			  ...
			  u8 out[SHA384_DIGEST_SIZE])
{
	...
	fips_lib_hmac_sha384_init(&ctx, key);
	fips_lib_hmac_sha384_update(&ctx, data, data_len);
	fips_lib_hmac_sha384_final(&ctx, out);
}
			  ...
			  u8 out[SHA512_DIGEST_SIZE])
{
	...
	fips_lib_hmac_sha512_init(&ctx, key);
				      ...
				      u8 out[SHA384_DIGEST_SIZE])
{
	...
	fips_lib_hmac_sha384_init_usingrawkey(&ctx, raw_key, raw_key_len);
	fips_lib_hmac_sha384_update(&ctx, data, data_len);
	fips_lib_hmac_sha384_final(&ctx, out);
}
