...
/*
 * fips_module private copy of lib/crypto/sha1.c
 *
 * Provides fips_lib_sha1_*() and fips_lib_hmac_sha1_*() — module-private
 * implementations of SHA-1 and HMAC-SHA1 library functions for use within
 * the FIPS cryptographic boundary.  These functions are not exported to
 * vmlinux; all callers within fips_module must include
 * "lib/crypto/sha1_lib.h" and call the fips_lib_* variants directly.
 *
...
void fips_lib_sha1_transform(__u32 *digest, const char *data, __u32 *array)
{
	__u32 A, B, C, D, E;
	unsigned int i = 0;
				...
				const u8 *data, size_t nblocks)
{
	...
	do {
		fips_lib_sha1_transform(state->h, data, workspace);
...
void fips_lib_sha1(const u8 *data, size_t len, u8 out[SHA1_DIGEST_SIZE])
{
	...
	fips_lib_sha1_init(&ctx);
	fips_lib_sha1_update(&ctx, data, len);
	fips_lib_sha1_final(&ctx, out);
}
				   ...
				   const u8 *raw_key, size_t raw_key_len)
{
	...
	if (unlikely(raw_key_len > SHA1_BLOCK_SIZE))
		fips_lib_sha1(raw_key, raw_key_len, derived_key.b);
	else
			...
			u8 out[SHA1_DIGEST_SIZE])
{
	...
	fips_lib_hmac_sha1_init(&ctx, key);
