 ...
 *
 * This is a private copy of lib/crypto/sha256.c for use within the FIPS
 * cryptographic module boundary.  All externally-visible function names carry
 * a fips_lib_ prefix so that intra-module references unambiguously resolve to
 * this implementation rather than the kernel's exported symbols.
 *
 * Architecture-specific sha256_blocks optimisations are intentionally omitted;
 * the generic C implementation is always used here.  The registered shash
 * algorithms in crypto/sha256.c use the same underlying code and are tested
 * by the full testmgr KATs at module load time.
 *
...
void fips_lib_sha224(const u8 *data, size_t len, u8 out[SHA224_DIGEST_SIZE])
{
	...
	fips_lib_sha224_init(&ctx);
	fips_lib__sha256_update(&ctx.ctx, data, len);
	fips_lib_sha224_final(&ctx, out);
}
...
void fips_lib_sha256(const u8 *data, size_t len, u8 out[SHA256_DIGEST_SIZE])
{
	...
	fips_lib_sha256_init(&ctx);
	fips_lib__sha256_update(&ctx.ctx, data, len);
	fips_lib_sha256_final(&ctx, out);
}
	...
	size_t len, u8 out1[SHA256_DIGEST_SIZE], u8 out2[SHA256_DIGEST_SIZE])
{
	...
	mut_ctx = *ctx;
	fips_lib__sha256_update(&mut_ctx, data1, len);
	__sha256_final(&mut_ctx, out1, SHA256_DIGEST_SIZE);

	mut_ctx = *ctx;
	fips_lib__sha256_update(&mut_ctx, data2, len);
				     ...
				     const struct sha256_block_state *iv)
{
	...
	if (unlikely(raw_key_len > SHA256_BLOCK_SIZE)) {
		if (iv == &sha224_iv)
			fips_lib_sha224(raw_key, raw_key_len, derived_key.b);
		else
			fips_lib_sha256(raw_key, raw_key_len, derived_key.b);
	} else {
			  ...
			  u8 out[SHA224_DIGEST_SIZE])
{
	...
	fips_lib__hmac_sha256_init(&ctx.ctx, &key->key);
	fips_lib__sha256_update(&ctx.ctx.sha_ctx, data, data_len);
	fips_lib_hmac_sha224_final(&ctx, out);
}
				      ...
				      u8 out[SHA224_DIGEST_SIZE])
{
	...
	fips_lib_hmac_sha224_init_usingrawkey(&ctx, raw_key, raw_key_len);
	fips_lib__sha256_update(&ctx.ctx.sha_ctx, data, data_len);
	fips_lib_hmac_sha224_final(&ctx, out);
}
