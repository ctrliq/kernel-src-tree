...
Users must link:{{< ref go_tools_compile.adoc >}}[install usr/bin/go] on their systems before building ‘lab’.  Users can build ‘lab’ by executing,
...
The next step is to enter a link:{{< ref RH_and_GitLab_Configuration.adoc >}}#gitLab-personal-access-tokens[GitLab Personal Access Token (PAT)]:
...
A project link:{{< ref what_is_a_GitLab_fork.adoc >}}[can be forked] by executing
...
----
	...
	# See link:{{< ref RH_and_GitLab_Configuration.adoc >}}[Red Hat and GitLab Configuration] for information.
	cd kernel-test
...
The third command, ‘lab mr create’ will open a $GIT_EDITOR window and provide an opportunity to change your merge request description and link:{{< ref verifying_a_gitlab_MR.adoc >}}[verify the actions you are making].  This is one of the reasons the two step merge request process may be preferable to users submitting multi-commit merge requests.
