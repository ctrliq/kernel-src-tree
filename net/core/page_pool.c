...
static bool page_pool_dma_map(struct page_pool *pool, struct page *page, gfp_t gfp)
{
	...
	if (in_softirq())
		err = xa_alloc(&pool->dma_mapped, &id, page,
	...
	page_pool_set_dma_index(page, id);
						 ...
						 gfp_t gfp)
{
	...
	if (pool->dma_map && unlikely(!page_pool_dma_map(pool, page, gfp))) {
		put_page(page);
		return NULL;
	}
						 ...
						 gfp_t gfp)
{
	...
	for (i = 0; i < nr_pages; i++) {
		...
		if (dma_map && unlikely(!page_pool_dma_map(pool, page, gfp))) {
			put_page(page);
			continue;
		}
...
void __page_pool_release_page_dma(struct page_pool *pool, struct page *page)
{
	...
	id = page_pool_get_dma_index(page);
	if (!id)
		return;
	...
	dma_unmap_page_attrs(pool->p.dev, dma,
			     ...
			     DMA_ATTR_SKIP_CPU_SYNC | DMA_ATTR_WEAK_ORDERING);
	page_pool_set_dma_addr(page, 0);
	page_pool_set_dma_index(page, 0);
}
