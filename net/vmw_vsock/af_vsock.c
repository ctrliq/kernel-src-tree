			...
			size_t len, int flags)
{
	...
	if (prot != &vsock_proto)
		return prot->recvmsg(sk, msg, len, flags & MSG_DONTWAIT,
...
static int vsock_net_mode_string(struct ctl_table *table, int write,
				 void *buffer, size_t *lenp, loff_t *ppos)
{
	struct net *net;
...
static int vsock_net_child_mode_string(struct ctl_table *table, int write,
				       void *buffer, size_t *lenp, loff_t *ppos)
{
	enum vsock_net_mode new_mode;
	struct net *net;
