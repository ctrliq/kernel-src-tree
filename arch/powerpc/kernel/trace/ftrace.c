...
static int ftrace_init_ool_stub(struct module *mod, struct dyn_ftrace *rec)
{
	...
	ppc_inst_t inst;
	/*
	 * See ftrace_64_mprofile.S if changing the below instruction sequence, as we rely on
...
{
	/* patch the address of ftrace_ops */
	unsigned long val = (unsigned long)ops;

	if (IS_ENABLED(CONFIG_PPC_FTRACE_OUT_OF_LINE))
		return patch_instructions((u32 *)
					  (ftrace_get_ool_stub(rec) - sizeof(unsigned long)),
					  (u32 *)&val, sizeof(unsigned long), false);
	else
		return patch_instructions((u32 *)
					  (rec->ip - MCOUNT_INSN_SIZE - sizeof(unsigned long)),
					  (u32 *)&val, sizeof(unsigned long), false);
}
