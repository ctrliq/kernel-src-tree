...
void resctrl_cpu_detect(struct cpuinfo_x86 *c)
{
	if (!cpu_has(c, X86_FEATURE_CQM_LLC)) {
		...
		c->_rh.x86_cache_mbm_width_offset = -1;
		return;
	}
	...
	if (cpu_has(c, X86_FEATURE_CQM_OCCUP_LLC) ||
		...
		c->_rh.x86_cache_mbm_width_offset = eax & 0xff;

		if (c->x86_vendor == X86_VENDOR_AMD && !c->_rh.x86_cache_mbm_width_offset)
			c->_rh.x86_cache_mbm_width_offset = MBM_CNTR_WIDTH_OFFSET_AMD;
	}
}
