...
static int init_tdx_module(void)
{
	...
	if (!(ret = read_sys_metadata_field(0x0800000100000005, &value)))
		update = value;
	if (!ret && !(ret = read_sys_metadata_field(0x0800000100000006, &value)))
		internal = value;
	if (!ret && !(ret = read_sys_metadata_field(0x8800000100000002, &value)))
		build_num = value;
	if (!ret && !(ret = read_sys_metadata_field(0x8800000200000001, &value)))
		build_date = value;

	if (!ret && !(ret = read_sys_metadata_field(0x0800000100000004, &value)))
		major = value;
	if (!ret && !(ret = read_sys_metadata_field(0x0800000100000003, &value)))
		minor = value;
	if (ret)
		return ret;

	pr_info("TDX module %d.%d.%02d.%02d, build number %d, build date %08d\n",
