...
/*
 * RHEL: We focus on memory hotplug; change the default to not trigger
 * actual hotunplug using alloc_contig_range(), which will taint the kernel.
 */
...
static int virtio_mem_fake_offline(unsigned long pfn, unsigned long nr_pages)
{
	...
	/*
	 * Memory hotunplug, when we actually have to call into
	 * alloc_contig_range() to evacuate/migrate memory, is disabled as
	 * default and might be unreliable in some cases RHEL 8.
	 */
	if (!printed) {
		printed = true;
		mark_tech_preview("virtio_mem memory hotunplug", THIS_MODULE);
	}
