					    ...
					    struct list_head *list)
{
	...
	if (WARN_ON(!list_empty(&dev->links_defer_sync)))
		return;

	get_device(dev);
	list_add_tail(&dev->links_defer_sync, list);
}
...
void device_links_supplier_sync_state_resume(void)
{
	...
	list_for_each_entry_safe(dev, tmp, &deferred_sync, links_defer_sync) {
		/*
...
static int dev_uevent_filter(struct kset *kset, const struct kobject *kobj)
{
...
static const char *dev_uevent_name(struct kset *kset, const struct kobject *kobj)
{
	const struct device *dev = kobj_to_dev(kobj);
