...
void sd_zbc_free_zone_info(struct scsi_disk *sdkp)
{
	...
	sdkp->aux->early_zone_info = (struct zoned_disk_info){ };
...
int sd_zbc_revalidate_zones(struct scsi_disk *sdkp)
{
	...
	u32 zone_blocks = sdkp->aux->early_zone_info.zone_blocks;
	unsigned int nr_zones = sdkp->aux->early_zone_info.nr_zones;
	u32 max_append;
	int ret = 0;
	unsigned int flags;
...
int sd_zbc_read_zones(struct scsi_disk *sdkp, u8 buf[SD_BUF_SIZE])
{
	...
	sdkp->aux->early_zone_info.nr_zones = nr_zones;
	sdkp->aux->early_zone_info.zone_blocks = zone_blocks;
