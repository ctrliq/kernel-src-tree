			  ...
			  struct gve_rx_alloc_rings_cfg *rx_alloc_cfg)
{
	 ...
	 * A count of 0 (a small device page budget shrinking a QPL to nothing)
	 * is rejected for the same reason: the buffer-state seeding loops
	 * would index entry -1 of a zero-length allocation.
	 */
	if (!tx_page_count || !rx_page_count || rx_page_count > U16_MAX) {
		err = -EINVAL;
		goto free_qpl_map;
	}
