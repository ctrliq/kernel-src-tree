				       ...
				       bool before_fork)
{
	...
	if(before_fork){
		/*
		 *  RHEL 8 does not handle this case correctly and the tests will
		 * fail. However, it's harder to fix in RHEL 8, the issue
		 * always existed in RHEL 8, and it's not considered a security
		 * problem.
		 */
		ksft_test_result_skip("Expected failure on RHEL 8\n");
		return;
	}
