...
Source8001: ciq_sb_ca.der
Source8002: ciq_sb_kernel.crt
...
%define pesign_uki_name_0 ciq_sb_uki
%define uki_secureboot_key_0 %{SOURCE8008}
%endif
%ifarch aarch64
...
%endif
    ...
    # need to use: Arch=`head -1 .config | cut -b 3-`
    # however arch is currently missing in all our config
...
%endif
    ...
    # create the kernel ABI metadata for use in packaging
...
# CIQ UKI Kernel signing - override above
%if 0%{?pe_uki_signing_certkeyslot:1}
        UKI_secureboot_name=%{pesign_uki_name_0}
        UKI_secureboot_cert=%{uki_secureboot_key_0}

	%pesign_uki -s -i $KernelUnifiedImage -o $KernelUnifiedImage.signed -a %{secureboot_ca_0} -c $UKI_secureboot_cert -n $UKI_secureboot_name
%else
...
# CIQ UKI Addon Signing
%if 0%{?pe_uki_signing_certkeyslot:1}
        %pesign_uki -s -i $addon -o $addon.signed -a %{secureboot_ca_0} -c $UKI_secureboot_cert -n $UKI_secureboot_name
%else
...
if [[ ! -e "/boot/symvers-%{KVERREL}%{?-v:+%{-v*}}.gz" ]]; then\
    ...
    if command -v restorecon &>/dev/null; then\
        restorecon "/boot/symvers-%{KVERREL}%{?-v:+%{-v*}}.gz"\
    fi\
fi\
%{nil}
...
#
%changelog
* Sat Sep 26 2026 github-actions[bot] <41898282+github-actions[bot]@users.noreply.github.com> - 6.18.54-1.1
-- Rebased changes for Linux 6.18.54 (https://github.com/ctrliq/kernel-src-tree/releases/tag/ciq_kernel-6.18.54-1)
-- [CIQ] v6.18.53 - updated spec (Brett Mastbergen)
...
-- bpf: Add BPF_F_ADJ_ROOM_DECAP_* flags for tunnel decapsulation (Brett Mastbergen)
...
-- [CIQ] v6.18.9 - rebased configs (github-actions[bot])
-- [CIQ] v6.18.7 - rebased configs (github-actions[bot])
-- [CIQ] v6.18.3 - rebased configs (github-actions[bot])
...
-- Linux 6.18.54 (https://cdn.kernel.org/pub/linux/kernel/v6.x/ChangeLog-6.18.54)
