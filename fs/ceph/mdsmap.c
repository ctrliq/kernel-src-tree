 ...
 * them).
 */
struct ceph_mdsmap *ceph_mdsmap_decode(struct ceph_mds_client *mdsc,
				       void **p, void *end, bool msgr2)
{
	...
	*p += sizeof(u8);			/* mdsmap_cv */
	...
	if (mdsmap_ev >= 8) {
		...
		/* fs_name */
		m->m_fs_name = ceph_extract_encoded_string(p, end, &fsname_len,
							   GFP_NOFS);
		...
		if (!namespace_equals(mdsc->fsc->mount_options, m->m_fs_name,
				      fsname_len)) {
			pr_warn("fsname %s doesn't match mds_namespace %s\n",
				m->m_fs_name,
