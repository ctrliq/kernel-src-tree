...
static void smbd_disconnect_rdma_connection(struct smbdirect_socket *sc)
{
	 ...
	 * disable[_delayed]_work_sync()
	 */
	cancel_work(&sc->recv_io.posted.refill_work);
	cancel_work(&sc->mr_io.recovery_work);
	cancel_work(&sc->idle.immediate_work);
	cancel_delayed_work(&sc->idle.timer_work);
...
wait_send_queue:
	...
	for_each_sg(sgl, sg, num_sgs, i) {
		request->sge[i+1].addr =
			ib_dma_map_page(sc->ib.dev, sg_page(sg),
			       sg->offset, sg->length, DMA_TO_DEVICE);
		if (ib_dma_mapping_error(
				sc->ib.dev, request->sge[i+1].addr)) {
			rc = -EIO;
			request->sge[i+1].addr = 0;
			goto err_dma;
		}
		request->sge[i+1].length = sg->length;
		request->sge[i+1].lkey = sc->ib.pd->local_dma_lkey;
		request->num_sge++;
	}
...
again:
	 ...
	 */
	log_read(INFO, "size=%u sc->recv_io.reassembly.data_length=%d\n", size,
		...
		unsigned int to_read)
{
	...
	ret = wait_event_interruptible(
		sc->recv_io.reassembly.wait_queue,
		sc->recv_io.reassembly.data_length >= to_read ||
			sc->status != SMBDIRECT_SOCKET_CONNECTED);
	if (ret)
		return ret;
...
static int allocate_mr_list(struct smbdirect_socket *sc)
{
	...
	for (i = 0; i < sp->responder_resources * 2; i++) {
		...
		mr->sgl = kcalloc(sp->max_frmr_depth,
