...
int cifs_wait_mtu_credits(struct TCP_Server_Info *server, unsigned int size,
...
ssize_t cifs_discard_from_socket(struct TCP_Server_Info *server,
				 size_t to_read);
int cifs_read_page_from_socket(struct TCP_Server_Info *server,
...
int setup_aio_ctx_iter(struct cifs_aio_ctx *ctx, struct iov_iter *iter,
		       int rw);
