...
func RHMAINTAINERS_header() {
	...
	fmt.Printf("%s","	   Supported:	Someone is actually paid to look after this.\n")
	fmt.Printf("%s","	   Maintained:	Someone actually looks after it.\n")
	fmt.Printf("%s","	   Odd Fixes:	It has a maintainer but they don't have time to do\n")
	fmt.Printf("%s","			much other than throw the odd patch in. See below..\n")
	fmt.Printf("%s","	   Orphan:	No current maintainer [but maybe you could take the\n")
	fmt.Printf("%s","			role as you write your new code].\n")
	fmt.Printf("%s","	   Obsolete:	Old code. Something tagged obsolete generally means\n")
	fmt.Printf("%s","			it has been replaced by a better system and you\n")
	fmt.Printf("%s","			should be using that.\n")
