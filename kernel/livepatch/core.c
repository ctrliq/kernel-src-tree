					...
					struct klp_object *obj)
{
	...
	mutex_lock(&module_ext_mutex);
	pmod_ext = find_module_ext(pmod);
	mutex_unlock(&module_ext_mutex);

	/* For each klp relocation section */
	for (i = 1; i < pmod_ext->klp_info->hdr.e_shnum; i++) {
		sec = pmod_ext->klp_info->sechdrs + i;
		secname = pmod_ext->klp_info->secstrings + sec->sh_name;
		...
		ret = apply_relocate_add(pmod_ext->klp_info->sechdrs,
					 pmod->core_strtab,
					 pmod_ext->klp_info->symndx, i, pmod);
		if (ret)
			break;
	}
