...
/*
 * This enables the per-console-device printk
 * kernel threads on CONFIG_PREEMPT_RT.
 *
 * In the case of !CONFIG_PREEMPT_RT, disable the kernel threads.
 * Disabling said threads by default on !CONFIG_PREEMPT_RT
 * kernels eliminates a class of init bugs caused by the threaded
 * console implementation.
 *
 * As a note, per-console-device printk kernel threads *cannot*
 * be disabled on CONFIG_PREEMPT_RT.
 */
...
static int __init printk_activate_kthreads(void)
{
	...
	/* Only enable the per-console device printk kernel threads on RT. */
	if (!IS_ENABLED(CONFIG_PREEMPT_RT))
		return 0;
